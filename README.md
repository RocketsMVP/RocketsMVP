# Hi there 👋

<!-- 仓库状态统计 -->
![Stars](https://img.shields.io/github/stars/RocketsMVP/RocketsMVP?style=flat-square&label=Stars&color=informational)
![Forks](https://img.shields.io/github/forks/RocketsMVP/RocketsMVP?style=flat-square&label=Forks&color=ff69b4)
![Watchers](https://img.shields.io/github/watchers/RocketsMVP/RocketsMVP?style=flat-square&label=Watchers&color=9cf)
![仓库大小](https://img.shields.io/github/repo-size/RocketsMVP/RocketsMVP?style=flat-square&color=success)
![最近提交](https://img.shields.io/github/last-commit/RocketsMVP/RocketsMVP/main?style=flat-square&label=Last%20Commit&color=blueviolet)
![License](https://img.shields.io/github/license/RocketsMVP/RocketsMVP?style=flat-square&label=License&color=yellow)

<!-- 主页访问量统计 -->
![Profile Views](https://komarev.com/gh-profile-views-badge/?username=RocketsMVP&style=flat-square&color=ff6b6b)

---

## 关于我

- 🔭 我目前正在做 **无人机系统** 开发
- 🌱 我是一名 **萌新开发者**，正在努力成长中
- 🛠 技术栈：Python / FastAPI / Vue / AI

## 正在做的项目

- **无人机应急响应系统** — 融合 AI 态势感知、路径规划、多部门协同处置的无人机管理平台

## 目标

- 持续学习全栈开发
- 深入无人机与 AI 结合的技术方向
- 努力写出更规范的代码

## 联系我

- GitHub: [RocketsMVP](https://github.com/RocketsMVP)

---

## 团队成员

### 👨‍💻 汪浩 - 后端开发

**教育背景**
- 重庆理工大学 软件工程(本科) 2022.9-2026.6

**专业技能**
- 熟练掌握 Java 语言特性，包括集合框架、多线程、并发工具类、异常处理、反射机制等
- 熟悉 JVM 原理，具备 JVM 参数调优、内存模型分析、GC 调优与性能问题排查经验
- 熟练掌握 MySQL，具备 SQL 编写、索引优化、事务管理与性能调优能力
- 熟悉 Redis 的常见数据结构及高可用部署方案，能进行缓存设计与优化
- 熟悉 Elasticsearch 的搜索、聚合功能及调优手段，能应对高并发检索场景
- 熟悉 Linux 操作系统，能进行常规运维、部署与故障排查
- 掌握前端框架 React、Vue 的基础开发，能独立实现前后端分离项目

**项目经验**
- 仿照"课堂派"的在线课程管理系统（Java、Spring Boot、MyBatis、MySQL、Redis）
- 仿照淘宝的电商商城（ArkTS、Java 后端、MySQL）
- Windows 文件管理器（Java、树形数据结构）
- 急救车物资管理系统（UML、数据库设计、Axure 原型设计）

**证书/荣誉**
- CET-4 级证书、CET-6 级证书
- 重庆华鸿公司华为鸿蒙实习训练营优秀学员
- 校学业三等奖学金
- 第十五届全国大学生数学竞赛省部级二等奖
- 第十六届全国大学生数学竞赛省部级三等奖

**联系方式**
- 📧 邮箱：956146885@qq.com
- 📱 电话：18987260800

---

*Keep flying, keep coding.*
